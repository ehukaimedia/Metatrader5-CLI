...
TRADE_RETCODE_PLACED = mt5.TRADE_RETCODE_PLACED
TRADE_RETCODE_INVALID_FILL = mt5.TRADE_RETCODE_INVALID_FILL
...
def connect(login=None, password=None, server=None, timeout: int = 60000) -> None:
    """Connect to the MT5 terminal.

    Idempotent — a second call while already connected is a no-op.
    Registers ``_shutdown`` via ``atexit`` on first successful connection.
    Raises ``ConnectionError`` if ``mt5.initialize()`` returns False.

    CRITICAL MT5 QUIRK: when password is None, call mt5.initialize() with NO
    args. Calling mt5.initialize(password=None) raises "Invalid password
    argument". Use bare initialize() for zero-credentials connections.
    """
    global _initialized
    if _initialized:
        return
...
def _shutdown() -> None:
    """Shut down the MT5 connection (called via atexit)."""
    global _initialized
    with _lock:
        mt5.shutdown()
        _initialized = False
