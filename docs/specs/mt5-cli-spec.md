...
| `analyze sniper-poc` | `SYMBOL --direction auto\|buy\|sell --bars INT --max-spread-points INT --min-rr FLOAT --entry-buffer-points INT --min-stop-points INT --stop-buffer-pips FLOAT --max-fvg-age-bars INT --max-sweep-age-bars INT --max-entry-distance-pips FLOAT --include-partial-fvg --allow-rollover --summary` | `status`, `direction`, `quote`, `dom`, `bias_counts`, `gates`, `setup`, `frames` or `frames_omitted`, `visual_contract` | Non-mutating M1 sniper point-of-confluence limit planner. Uses H4/H1/M15/M5 structure majority, fresh M1/M5/M15 OPEN FVG midpoints, recent liquidity sweep/target context, DOM when available, and current bid/ask quote rules. For sniper sweep detection it uses tighter Ehukai liquidity pivots on M1/M5 (`length=5`) and context pivots on M15+ (`length=14`), then gates against true `sweep_age_bars`. Returns `candidate` only when spread, rollover, quote-side, liquidity, stop-distance, and R:R gates pass; otherwise returns `no_trade` with failed gates. Buy limits are validated against ask-trigger behavior and sell limits against bid-trigger behavior. SL is widened to at least `--min-stop-points`. Auto-direction ties resolve to `no_trade`. A candidate includes `setup.order_commands`: first `order dryrun --order-type limit`, then `order limit`. |
...
- `analyze sniper-poc` is not a dryrun. A `candidate` only means the analysis gates passed at that quote; run the returned `order dryrun --order-type limit` command before placement, then re-check `order list`/quote freshness.
