...
| Command | Args | JSON output keys | Description |
|---------|------|-----------------|-------------|
| `chart list` | `--window TEXT` | `[{chart_id, title, hwnd, symbol, timeframe, active}]` | List MT5 MDI child charts so agents can target an already-open symbol chart without opening duplicates. |
| `chart switch-tf` | `TIMEFRAME` `--window TEXT` `--settle-seconds FLOAT` `--chart-id ID` | `timeframe`, `title`, `hwnd` | Switch the targeted or active MT5 chart timeframe through the period toolbar and verify the active child title. |
| `chart current` | `--window TEXT` `--chart-id ID` | `title`, `hwnd` | Read the targeted or active MT5 child chart title. Use this before GUI workflows to confirm the active chart. |
| `chart symbol` | `SYMBOL` `--window TEXT` `--settle-seconds FLOAT` `--chart-id ID` | `symbol`, `title`, `hwnd` | Activate an existing symbol child chart when present; otherwise switch the targeted or active MT5 chart symbol and verify the active child title. |
| `chart ensure` | `SYMBOL --timeframe TF --window TEXT --settle-seconds FLOAT --chart-id ID` | `symbol`, `timeframe`, `title`, `hwnd`, `chart_id` | Ensure the active MT5 chart is on the broker-exact SYMBOL and optional timeframe, then verify the title. Defaults to `M15`; use `--timeframe none` to only ensure the symbol. This is the preferred chart-selection primitive for agents. |
...
`chart ensure` must prefer activating an existing MDI child chart whose title matches the broker-exact symbol before falling back to symbol switching. MDI child activation must use `WM_MDIACTIVATE` on the terminal's `MDIClient` so tiled/cascaded chart layouts keep their original child window rectangles; child-level `BringWindowToTop()` / `ShowWindow()` calls are only fallback behavior when no MDI client is available. After activation it must read the active child title and verify the requested symbol/timeframe; `--chart-id` is an optional explicit target from `chart list` for ambiguous workspaces.
...
| `analyze sniper-poc` | `SYMBOL --direction auto\|buy\|sell --bars INT --max-spread-points INT --min-rr FLOAT --entry-buffer-points INT --min-stop-points INT --stop-buffer-pips FLOAT --max-fvg-age-bars INT --max-sweep-age-bars INT --max-entry-distance-pips FLOAT --include-partial-fvg --allow-rollover --summary` | `status`, `direction`, `quality_score`, `quote`, `dom`, `bias_counts`, `structure`, `poi`, `liquidity`, `entry`, `gates`, `setup`, `frames` or `frames_omitted`, `visual_contract` | Non-mutating sniper setup planner. Uses D1/H4/M15/M5 structure majority, fresh M1/M5/M15 FVG midpoints, recent sweep/target context, geometric POI trap checks, DOM when available, and current bid/ask quote rules. For sniper sweep detection it uses tighter Ehukai liquidity pivots on M1/M5 (`length=5`) and context pivots on M15+ (`length=14`), then gates against true `sweep_age_bars`. POI trap geometry separates entry-distance filtering from a local behind-zone tolerance so distant liquidity does not distort trap risk. Returns `ready` only when spread, rollover, quote-side, POI, liquidity, stop-distance, R:R, and M1/M5 entry-structure gates pass; returns `watch` when a setup is valid but entry structure is not confirmed; otherwise returns `no_trade` with failed gates. Buy limits are validated against ask-trigger behavior and sell limits against bid-trigger behavior. SL is widened to at least `--min-stop-points`. Auto-direction ties resolve to `no_trade`. A ready setup includes `setup.order_commands`: first `order dryrun --order-type limit`, then `order limit`. |
...
| `order list` | `--symbol TEXT` `--strategy-id TEXT` | `[{ticket, symbol, type, volume_initial, volume_current, price_open, price_current, sl, tp, state, type_filling, type_filling_name, magic, is_agent_magic, strategy_id, comment, comment_truncated}]` | Current pending orders from `orders_get`. Use this before relying on chart-only trade-panel reads. `strategy_id` only reverse-resolves from configured `strategy_ids` or a `--strategy-id` filter; `is_agent_magic` marks auto-derived strategy-range magics even when the name is unknown. Broker comments can truncate, so agents must not parse `comment` as identity. |
...
| `order ready-limit` | `SYMBOL --direction auto\|buy\|sell --volume FLOAT --risk-pct FLOAT --strategy-id TEXT --max-entry-drift-points INT --filling {FOK,IOC,RETURN,auto}` plus sniper setup filters | `status`, `initial_setup`, `final_setup`, `dryrun`, `placement`, `safety` | Supervised TDA limit placement. Calls `analyze sniper-poc`, requires `status=ready`, runs a broker `order dryrun`, refreshes setup/quote, rejects entry drift beyond `--max-entry-drift-points`, runs an immediate second dry-run, then places a limit order through the normal guarded `order limit` path. Does not add a demo-only or account-type block; existing `--live` / `MT5_LIVE=1` live-intent and risk gates still apply. |
...
| `order dryrun` | (same as `order market`) plus `--order-type market\|limit\|stop --price FLOAT` for pending checks | `margin`, `margin_free`, `margin_level`, `profit`, `retcode`, `dry_run: true`, `order_type`, `price` | Pre-flight check via `mt5.order_check()` — broker-validated, no order sent. For pending limit/stop checks, pass `--order-type` and `--price`. |
...
- `analyze sniper-poc` is not a dryrun. `ready` only means the analysis gates passed at that quote. For manual placement, run the returned `order dryrun --order-type limit` command before placement, then re-check `order list`/quote freshness.
- `order ready-limit` is the preferred supervised bridge from a READY setup to a real pending limit order. It intentionally does not block accounts by reported account type because broker demo accounts can report as live-capable accounts. It still requires explicit live intent when the existing risk gate sees a real/live-capable account.
