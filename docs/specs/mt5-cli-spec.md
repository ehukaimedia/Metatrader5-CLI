...
- `analyze sniper-poc` is not a dryrun. `ready` only means the analysis gates passed at that quote; run the returned `order dryrun --order-type limit` command before placement, then re-check `order list`/quote freshness.
