...
|---|---|---|
| `place_stop(symbol, side, price, volume, sl, tp=None, *, cfg, is_live_intent, strategy_id=None)` | `order_send` with `TRADE_ACTION_PENDING` + `ORDER_TYPE_BUY_STOP / SELL_STOP` | Same risk-gate keyword-only pattern as `place_limit`. `cfg` required so `check_order` Guard 2 can enforce the live triple lock (`cfg["live"]` + `MT5_LIVE=1` + `is_live_intent`). Cherry-pick `archive/legacy-mt5/core/order.py::place_stop` (around line 369) — but pass `entry_price=float(price)` to `check_order` so the SL-distance gate measures from the pending trigger, not the current ask (Codex post-fix P1 #2). |
| `modify(ticket, *, sl=None, tp=None, price=None, expiry=None, cfg, is_live_intent)` | For open positions: `order_send` with `TRADE_ACTION_SLTP` + `position=ticket`. For pending orders: `TRADE_ACTION_MODIFY` + `order=ticket` | Single entry point handles both pending modify and position SL/TP update — detect via `positions_get(ticket=ticket)` first. `cfg` REQUIRED keyword: `_live_gate_check(is_live_intent, cfg)` enforces the triple lock on REAL accounts (`is_live_intent` + `cfg["live"]` + `MT5_LIVE=1`). Earlier drafts of this row omitted `cfg`, which silently bypassed `cfg["live"]` — the exact bug Codex post-fix P1 closed at `62c7081`. Do not regress. |
| `cancel_all_pending(symbol=None, *, cfg, is_live_intent)` | Iterate `orders_get(symbol=symbol)`, call `cancel(ticket, cfg=cfg, is_live_intent=is_live_intent)` per-ticket, fail-soft per-ticket | Returns `ok({"per_ticket": [{ticket, ok, error}, ...], "cancelled": N, "failed": N})`. `cfg` REQUIRED keyword AND threaded into each recursive `cancel()` call — otherwise the per-ticket cancels lose the triple-lock context. Same Codex post-fix P1 rationale as `modify` above. |
