    ...
    "legend": [
        {
            ...
            "label_pattern": "TDA v1.26 TOP-DOWN / GUIDE: <WAIT|WATCH|NO TRADE> ... / VP: <read>",
            "meaning": "Preferred visual presentation layer for screenshot agents and manual TDA. Manual mode shows a left-side top-down structure panel, current-timeframe swing labels, the latest close-confirmed BOS/CHOCH/iBOS rail with a compact marker, active historical FVG zones without text labels by default, subtle recent sweep hints, a compact VP/POC confluence rail, and a left-side GUIDE panel. M1/M5 use a smaller adaptive FVG threshold for entry work. Filled FVGs disappear while open or partially filled historical FVGs remain eligible and are selected by proximity to current price. Structure reads use the elite-v1 closed-bar contract: swing BOS/CHOCH determines trade permission, internal structure confirms entry-side pressure, FVG/POI supplies execution context, liquidity supplies sweep/trap context, and VP/POC supplies confluence only. Top-right status headers, dense FVG text labels, historical BOS/CHOCH text labels, full liquidity rails, full VP histogram, stale debug objects, and elite strong/weak rails stay visual-off by default while remaining available as structured context.",
            "structured_source": "ehukai structure + ehukai fvg + ehukai liquidity + ehukai volume_profile",
        },
        {
            "indicator": "EhukaiTDAOverlay",
            "visual": "manual trade guide label",
            "label_pattern": "GUIDE: <WAIT|WATCH|NO TRADE> ...",
            "meaning": "Checklist-style visual guide that summarizes bias, sweep context, nearest FVG POI, compact VP/POC read, and invalidation. It is a decision aid, not an automatic order trigger.",
            "structured_source": "EhukaiTDAOverlay setup context",
        },
        {
            "indicator": "EhukaiTDAOverlay",
            "visual": "top-down structure panel",
            "label_pattern": "TDA v1.26 TOP-DOWN / D1|H4|M15|M5|M1 <read>",
        ...
        {
            "indicator": "EhukaiTDAOverlay",
            "visual": "compact volume profile POC rails",
            "label_pattern": "VP: <inside value|above POC|below POC|POC blocks ...>",
            "meaning": "Volume Profile confluence inside the preferred overlay. POC is drawn as a subtle rail by default; VAH/VAL are optional. This layer helps identify value/chop, POC support, and POC obstruction, but does not replace market structure, FVG/POI, liquidity, or risk gates.",
            "structured_source": "EhukaiTDAOverlay VP context + ehukai.volume_profile",
        },
        {
            "indicator": "EhukaiTDAOverlay",
