...
This is the SECOND reviewer in the phase-2 autopilot consensus pipeline.
Together with `ClaudeReviewer`, the pair forms a strict 2-of-2 consensus.
The two reviewers MUST run different model families (Claude + Codex) for
independence — that's the whole reason this skill exists rather than
spawning two Claude instances.
...
- **Vote on the original levels.** Your job is to evaluate the
  ...
  operator; the phase-2 autopilot consensus treats anything except
  unanimous `take` with `accepted_levels=true` as an automatic skip.
...
4. Emit the verdict by writing
   `.ehukaiconnect/shared/files/verdicts/<alert_id>-codex.json`
   (note the `-codex` suffix — distinct from `-claude` so the consensus
   evaluator can pair them) with the schema below, then close the task:
...
- If you cannot reach a decision in 90s, close the task as `done` with
  `decision="skip"` and `reasoning_summary="timeout"`. The autopilot
  consensus rule treats this as no_consensus.
- The autopilot consensus pipeline treats anything except
  `decision="take"` with `accepted_levels=true` as a no-go, so be
  honest about uncertainty — vote skip rather than borderline take.

## Independence

You are deliberately NOT in lockstep with `ClaudeReviewer`. Disagreement
is informative — it surfaces noise vs. signal. Vote your honest read of
the chart and the deterministic setup, not what you think the other
reviewer will say.
