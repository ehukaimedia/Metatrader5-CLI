...
- `analyze sniper-poc` consumes Ehukai structure/FVG/liquidity context plus DOM or quote fallback to return `no_trade`, `watch`, or `ready`; it never places orders and returns dryrun-before-placement commands when a setup is ready. `order ready-limit` is the supervised bridge that can consume a READY setup, dry-run it, refresh it, and then place a guarded limit order.
...
mt5 --live --json order ready-limit USDJPY --direction auto --volume 0.001 --strategy-id ehukai-m1-sniper-poc --allow-rollover
```
...
- Treat a ready sniper POC setup as analysis only unless using `order ready-limit`. Manual agents must run the returned `order dryrun --order-type limit` command and then re-check quote/order freshness before sending the returned `order limit`; `order ready-limit` performs that dry-run/refresh/immediate-dry-run sequence itself.
- Do not add a demo-only account-type block to `order ready-limit`; keep the existing live-intent/risk gate because broker demo accounts can report as live-capable accounts.
